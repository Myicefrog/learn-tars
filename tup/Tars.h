...
do{ \
	std::cout<<"TarsReserveBuf len is "<<len<<std::endl; \
	std::cout<<"TarsReserveBuf (os)._buf_len is "<<(os)._buf_len<<std::endl; \
    if((os)._reverse) \
    { \
        if(tars_unlikely((os)._buf_len < (len))) \
        { \
            size_t len1 = (len)<<1; \
			std::cout<<"TarsReserveBuf len1 is "<<len1<<std::endl; \
...
do { \
    ...
    { \
		std::cout<<"TarsWriteToHead < 15 type "<<type<<" tag<<4 is"<<(int)((tag)<<4)<<std::endl; \
        TarsWriteUInt8TTypeBuf( os, (type) + ((tag)<<4) , (os)._len); \
    } \
    else \
    { \
		std::cout<<"TarsWriteToHead > 15 type "<<type<<" tag is"<<(int)(tag)<<std::endl; \
...
do { \
    ...
    (osLen) += sizeof(Char); \
	std::cout<<"TarsWriteCharTypeBuf val is "<<val<<std::endl; \
	std::cout<<"TarsWriteCharTypeBuf os.buf is "<<std::endl; \
	for(size_t i = 0; i < osLen; i++) \
		std::cout<<(os)._buf[i]<<std::endl; \
	std::cout<<"TarsWriteCharTypeBuf os.buf end"<<std::endl; \
} while(0)
...
do { \
	std::cout<<"TarsWriteUInt32TTypeBuf before osLen is "<<osLen<<std::endl; \
    TarsReserveBuf(os, (osLen)+sizeof(uint32_t)); \
    (*(uint32_t *)((os)._buf + (osLen))) = (val); \
    (osLen) += sizeof(uint32_t); \
	std::cout<<"TarsWriteUInt32TTypeBuf os._buf is "<<std::endl; \
	for(size_t i = 0; i < osLen; i++) \
		std::cout<<(os)._buf[i]<<std::endl; \
	std::cout<<"TarsWriteUInt32TTypeBuf os._buf end "<<std::endl; \
} while(0)

#define TarsWriteUInt8TTypeBuf(os, val, osLen) \
do { \
	std::cout<<"TarsWriteUInt8TTypeBuf before osLen is "<<osLen<<std::endl; \
	std::cout<<"TarsWriteUInt8TTypeBuf val is "<<val<<std::endl; \
    TarsReserveBuf(os, (osLen)+sizeof(uint8_t)); \
    (*(uint8_t *)((os)._buf + (osLen))) = (val); \
    (osLen) += sizeof(uint8_t); \
	std::cout<<"TarsWriteUInt8TTypeBuf after osLen is "<<osLen<<std::endl; \
	std::cout<<"TarsWriteUInt8TTypeBuf os._buf is "<<std::endl; \
	for(size_t i = 0; i < osLen; i++) \
		std::cout<<(os)._buf[i]<<std::endl; \
	std::cout<<"TarsWriteUInt8TTypeBuf os._buf end "<<std::endl; \
} while(0)
...
do { \
	std::cout<<"TarsWriteTypeBuf len is "<<len<<std::endl; \
    TarsReserveBuf(os, (os)._len + (len)); \
    memcpy((os)._buf + (os)._len, (const void *)(buf), (len)); \
    (os)._len += (len); \
	for(size_t i = 0; i < (os)._len; i++) \
		std::cout<<(os)._buf[i]<<std::endl; \
	std::cout<<"TarsWriteTypeBuf end "<<len<<std::endl; \
} while(0)
...
do { \
	...
	std::cout<<"TarsPeekTypeBuf buf is "<<(buf)<<std::endl; \
} while(0)
...
do {\
    (is)._cur += (len); \
	std::cout<<"TarsReadHeadSkip _cur is "<<(is)._cur<<std::endl; \
} while(0)

#define TarsPeekFromHead(is, type, tag, n) \
do { \
	std::cout<<"TarsPeekFromHead begin"<<std::endl; \
    (n) = 1; \
    uint8_t typeTag, tmpTag; \
    TarsPeekTypeBuf(is, typeTag, 0, uint8_t); \
	std::cout<<"TarsPeekFromHead typeTag "<<(int)typeTag<<std::endl; \
    tmpTag = typeTag >> 4; \
	std::cout<<"TarsPeekFromHead tmpTag "<<(int)tmpTag<<std::endl; \
    (type) = (typeTag & 0x0F); \
	std::cout<<"TarsPeekFromHead type "<<(int)type<<std::endl; \
    if(tars_unlikely(tmpTag == 15)) \
    { \
		std::cout<<"TarsPeekFromHead unlikely tmpTag "<<(int)tmpTag<<std::endl; \
...
do { \
    ...
    { \
        ...
        { \
            ...
            if (tag == nextHeadTag) \
            { \
				std::cout<<"TarsSkipToTag tag == nextHeadTag"<<std::endl; \
...
//////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        void skipField(uint8_t type)
        {
            ...
            {
            case TarsHeadeChar:
				std::cout<<"skipField "<<TarsHeadeChar<<std::endl;
                TarsReadHeadSkip(*this, sizeof(Char));
                break;
            case TarsHeadeShort:
				std::cout<<"skipField "<<TarsHeadeShort<<std::endl;
                TarsReadHeadSkip(*this, sizeof(Short));
                break;
            case TarsHeadeInt32:
				std::cout<<"skipField "<<TarsHeadeInt32<<std::endl;
                TarsReadHeadSkip(*this, sizeof(Int32));
                break;
            case TarsHeadeInt64:
				std::cout<<"skipField "<<TarsHeadeInt64<<std::endl;
                TarsReadHeadSkip(*this, sizeof(Int64));
                break;
            case TarsHeadeFloat:
				std::cout<<"skipField "<<TarsHeadeFloat<<std::endl;
                TarsReadHeadSkip(*this, sizeof(Float));
                break;
            case TarsHeadeDouble:
				std::cout<<"skipField "<<TarsHeadeDouble<<std::endl;
                TarsReadHeadSkip(*this, sizeof(Double));
                break;
            case TarsHeadeString1:
				std::cout<<"skipField "<<TarsHeadeString1<<std::endl;
                {
                    size_t len = 0;
            ...
            case TarsHeadeString4:
				std::cout<<"skipField "<<TarsHeadeString4<<std::endl;
                {
                    size_t len = 0;
                ...
                break;
            case TarsHeadeMap:
				std::cout<<"skipField "<<TarsHeadeMap<<std::endl;
                {
                    Int32 size = 0;
                    read(size, 0);
                ...
                break;
            case TarsHeadeList:
				std::cout<<"skipField "<<TarsHeadeList<<std::endl;
                {
                    Int32 size = 0;
                    read(size, 0);
            ...
            case TarsHeadeSimpleList:
				std::cout<<"skipField "<<TarsHeadeSimpleList<<std::endl;
                {
                    uint8_t headType = 0, headTag = 0;
            ...
            case TarsHeadeStructBegin:
				std::cout<<"skipField "<<TarsHeadeStructBegin<<std::endl;
                skipToStructEnd();
                break;
            case TarsHeadeStructEnd:
				std::cout<<"skipField "<<TarsHeadeStructEnd<<std::endl;
        ...
        void read(Short& n, uint8_t tag, bool isRequire = true)
        {
            ...
            if (tars_likely(skipFlag))
            {
                ...
                {
                case TarsHeadeZeroTag:
				std::cout<<"read Short TarsHeadeZeroTag"<<std::endl;
                    n = 0;
                    break;
                case TarsHeadeChar:
				std::cout<<"read Char TarsHeadeChar"<<std::endl;
                    TarsReadTypeBuf(*this, n, Char);
                    break;
                case TarsHeadeShort:
				std::cout<<"read Short TarsHeadeShort"<<std::endl;
...
//////////////////////////////////////////////////////////////////
    ...
    public:
        ...
        {
			std::cout<<"write Char "<<n<<" tag is "<<(int)tag<<std::endl;
            /*
        ...
        {
			std::cout<<"write Short "<<n<<" tag is "<<(int)tag<<std::endl;
        ...
        {
			std::cout<<"write Int32 "<<n<<" tag is "<<(int)tag<<std::endl;
        ...
        {
			std::cout<<"write Int64 "<<n<<" tag is "<<(int)tag<<std::endl;
        ...
        void write(const std::string& s, uint8_t tag)
        {
            if (tars_unlikely(s.size() > 255))
            {
				...
				std::cout<<"write string4 "<<" tag is "<<(int)tag<<std::endl;
            ...
            {
				std::cout<<"write string1 "<<" tag is "<<(int)tag<<std::endl;
        ...
        {
			std::cout<<"write std::map<> tag is "<<int(tag)<<std::endl;
        ...
        {
			std::cout<<" write vector<Char> tag is "<<int(tag)<<std::endl;
